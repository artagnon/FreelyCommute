...
inline std::ostream &operator<<(std::ostream &OS, const Point &Obj) {
  OS << std::string(3, ' ') << "{" << Obj.X << ", " << Obj.Y << ", "
     << Obj.Speed << ", " << Obj.Direction << ", " << Obj.Width << ", "
     << Obj.Pressure << "}" << std::endl;
  return OS;
}
inline std::ostream &operator<<(std::ostream &OS, const Line &Obj) {
  OS << std::string(2, ' ') << std::dec << Obj.NChildren
     << " points:" << std::endl;
  printChildren(OS, Obj);
  return OS;
}
inline std::ostream &operator<<(std::ostream &OS, const Layer &Obj) {
  OS << std::string(1, ' ') << std::dec << Obj.NChildren
     << " lines:" << std::endl;
  printChildren(OS, Obj);
  return OS;
}
